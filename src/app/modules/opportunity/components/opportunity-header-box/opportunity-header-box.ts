...
export class OpportunityHeaderBox {
  ...
  ngOnChanges() {
    {
      ...
      if (this.opportunity) {
        this.headerForm.patchValue({
          ...
          expectedCloseDate: this.opportunity.expectedCloseDate ? new Date(this.opportunity.expectedCloseDate).toISOString().substring(0, 16) : '',
        });
      }
    }
  }
  ...
  private setupFormChangeListeners() {
    ...
    this.headerForm.get('expectedCloseDate')?.valueChanges.subscribe(newValue => {
      if (this.opportunity && newValue !== this.getOriginalFieldValue('expectedCloseDate')) {
        this.requestFieldChangeConfirmation('expectedCloseDate', this.getOriginalFieldValue('expectedCloseDate'), newValue);
      }
    });
    ...
    this.headerForm.get('ownerId')?.valueChanges.subscribe(newValue => {
      if (this.opportunity && newValue !== this.getOriginalFieldValue('ownerId')) {
        this.requestFieldChangeConfirmation('ownerId', this.getOriginalFieldValue('ownerId'), newValue);
      }
    });
  }
  ...
  private getOriginalFieldValue(fieldName: string): any {
    ...
    switch (fieldName) {
      case 'expectedCloseDate':
        return this.opportunity.expectedCloseDate ? new Date(this.opportunity.expectedCloseDate).toISOString().substring(0, 16) : '';
      case 'ownerName':
  ...
  private requestFieldChangeConfirmation(fieldName: string, oldValue: any, newValue: any) {
    ...
    switch (fieldName) {
      case 'expectedCloseDate':
        this.confirmationTitle = 'Update Expected Close Date';
        this.confirmationMessage = `Are you sure you want to change the expected close date from "${oldValue || 'Not Set'}" to "${newValue}"?`;
        break;
      case 'ownerName':
      case 'ownerId':
  ...
  onChangeCancelled() {
    ...
    }, { emitEvent: false }); // emitEvent: false prevents triggering valueChanges again
  ...
  updateExpectedCloseDate() {
    ...
    const newDate = this.headerForm.value.expectedCloseDate;
    ...
    console.log('Updating expected close date with data:', updateData);
    this.opportunityService.updateOpportunity(this.opportunity.opportunityId, updateData).subscribe({
      next: (updatedOpportunity) => {
        console.log('Expected close date updated successfully:', updatedOpportunity);
  ...
  updateOwner() {
    ...
    this.opportunityService.updateOpportunity(updateData).subscribe({
      next: (updatedOpportunity) => {
